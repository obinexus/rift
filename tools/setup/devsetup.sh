#!/bin/bash
#
# RIFT Development Environment Setup Script
# =========================================
#
# This script sets up the development environment for the RIFT project.
...
check_tools() {
    ...
    if [ "$CMAKE_MAJOR" -lt 3 ] || ([ "$CMAKE_MAJOR" -eq 3 ] && [ "$CMAKE_MINOR" -lt 10 ]); then
        warning "CMake version $CMAKE_VERSION detected. RIFT requires CMake 3.10 or higher."
    ...
    if [ "$GCC_MAJOR" -lt 7 ]; then
        warning "GCC version $GCC_VERSION detected. RIFT recommends GCC 7.0 or higher for full C11 support."
    else
...
#!/bin/bash
# Pre-commit hook for RIFT project
...
#!/bin/bash
# commit-msg hook for RIFT project
...
if ! [[ "$commit_msg" =~ $conventional_pattern ]]; then
    ...
    echo "Example: feat(parser): add support for regex states"
    exit 1
fi
...
export CPPCHECK_SUPPRESS="\$RIFT_ROOT/.cppcheck_suppressions"
EOF
            ...
            if [ -n "$SHELL_PROFILE" ]; then
                ...
                if ! grep -q "source $ENV_FILE" "$SHELL_PROFILE"; then
                    echo "# RIFT environment setup" >> "$SHELL_PROFILE"
        ...
        windows)
            ...
            info "2. Set RIFT_ROOT environment variable to: %CD%"
            ;;
    esac
...
EOF
    ...
    success "format.sh script created/updated"
}

# Function to run initial build and tests
run_initial_build() {
    section "Running Initial Build and Tests"
    ...
    cd ../..
    success "Initial build and tests completed"
}
...
success "RIFT development environment has been successfully set up!"
info "You may need to restart your terminal or run 'source ~/.bashrc' (or equivalent) to apply environment changes."
info "Start developing with: cd build/debug && make"