...
bool rift_token_memory_load_stage0_file(RiftTokenMemory* memory, const char* rift0_file);
...
RiftTokenValidationResult rift_token_memory_get_validation_result(
    const RiftMemoryToken* token);
...
bool rift_token_memory_save_rift0(const RiftTokenMemory* memory, const char* filename);
bool rift_token_memory_save_rift1(const RiftTokenMemory* memory, const char* filename);
bool rift_token_memory_load_rift1(RiftTokenMemory* memory, const char* filename);

// Query and filtering operations
RiftMemoryToken** rift_token_memory_find_tokens_by_type(
    const RiftTokenMemory* memory, 
    RiftTokenType type, 
    size_t* result_count);
RiftMemoryToken** rift_token_memory_find_tokens_by_value(
    const RiftTokenMemory* memory, 
...
bool rift_token_memory_wait_for_processing(RiftTokenMemory* memory, int timeout_ms);
