...
RiftResult rift_ast_node_remove_child(RiftASTNode* parent, size_t index) {
    if (!parent || index >= parent->child_count) return RIFT_ERROR_INVALID_INPUT;
...
void rift_ast_print(RiftASTNode* root, int indent) {
    ...
    // Print node info
    printf("AST[%d]: %s\n", root->node_type, root->value ? root->value : "(null)");
...
RiftResult rift_automaton_add_state(RiftAutomaton* automaton, RiftState* state) {
    if (!automaton || !state) return RIFT_ERROR_NULL_POINTER;
    ...
    if (automaton->state_count >= automaton->state_capacity) {
        ...
        RiftState** new_states = realloc(automaton->states, 
                                       new_capacity * sizeof(RiftState*));
        if (!new_states) return RIFT_ERROR_MEMORY_ALLOCATION;
...
RiftPattern* rift_pattern_create(const char* pattern, RiftTokenType type) {
    ...
    if (pattern) {
        ...
        // Compile regex
        int result = regcomp(&pattern_obj->compiled_regex, pattern, REG_EXTENDED | REG_NOSUB);
        if (result != 0) {
...
RiftResult rift_token_memory_add_token(RiftTokenMemory* memory, RiftToken* token) {
    if (!memory || !token) return RIFT_ERROR_NULL_POINTER;

    // Resize if needed
    if (memory->token_count >= memory->token_capacity) {
        size_t new_capacity = memory->token_capacity * 2;
        RiftToken** new_tokens = realloc(memory->tokens, 
                                       new_capacity * sizeof(RiftToken*));
        if (!new_tokens) return RIFT_ERROR_MEMORY_ALLOCATION;
...
RiftToken* rift_token_create(const char* type, const char* value, const char* lexeme) {
    ...
    if (type) {
        ...
        if (!token->type) goto error;
    }

    if (value) {
        token->value = strdup(value);
        if (!token->value) goto error;
    }

    if (lexeme) {
        token->lexeme = strdup(lexeme);
        if (!token->lexeme) goto error;
    }
...
RiftResult rift_ir_read_file(RiftIR* ir, const char* filename) {
    if (!ir || !filename) return RIFT_ERROR_NULL_POINTER;
...
void rift_ir_write_ast_node(FILE* file, RiftASTNode* node, int indent) {
    ...
    // Write node
    fprintf(file, "node:%d:%s\n", node->node_type, node->value ? node->value : "");
...
const char* rift_result_to_string(RiftResult result) {
    switch (result) {
        ...
        case RIFT_ERROR_NULL_POINTER: return "ERROR_NULL_POINTER";
        case RIFT_ERROR_MEMORY_ALLOCATION: return "ERROR_MEMORY_ALLOCATION";
        case RIFT_ERROR_FILE_NOT_FOUND: return "ERROR_FILE_NOT_FOUND";
        case RIFT_ERROR_INVALID_INPUT: return "ERROR_INVALID_INPUT";
        case RIFT_ERROR_PARSE_FAILED: return "ERROR_PARSE_FAILED";
        case RIFT_ERROR_PARSE_INCOMPLETE: return "ERROR_PARSE_INCOMPLETE";
        case RIFT_ERROR_PATTERN_INVALID: return "ERROR_PATTERN_INVALID";
        case RIFT_ERROR_INVALID_PARSE_MODE: return "ERROR_INVALID_PARSE_MODE";
        case RIFT_ERROR_PARSING_INCOMPLETE: return "ERROR_PARSING_INCOMPLETE";
...
const char* rift_token_type_to_string(RiftTokenType type) {
    switch (type) {
        case RIFT_TOKEN_UNKNOWN: return "UNKNOWN";
        case RIFT_TOKEN_IDENTIFIER: return "IDENTIFIER";
        case RIFT_TOKEN_INTEGER: return "INTEGER";
        case RIFT_TOKEN_FLOAT: return "FLOAT";
        case RIFT_TOKEN_STRING: return "STRING";
        case RIFT_TOKEN_CHAR: return "CHAR";
        case RIFT_TOKEN_OPERATOR: return "OPERATOR";
        case RIFT_TOKEN_DELIMITER: return "DELIMITER";
        case RIFT_TOKEN_SEPARATOR: return "SEPARATOR";
        case RIFT_TOKEN_KEYWORD: return "KEYWORD";
        case RIFT_TOKEN_WHITESPACE: return "WHITESPACE";
        case RIFT_TOKEN_EOF: return "EOF";
...
void rift_debug_print_token(RiftToken* token) {
    ...
    printf("Token: type='%s' value='%s' lexeme='%s' type_enum=%s line=%zu col=%zu\n",
           token->type ? token->type : "(null)",
           token->value ? token->value : "(null)",
           token->lexeme ? token->lexeme : "(null)",
           rift_token_type_to_string(token->type_enum),
           token->line,
           token->column);
}
...
RiftASTNode* rift1_parse_keyword_statement(Rift1ParseEngine* engine) {
    ...
    RiftASTNode* statement = rift_ast_node_create(AST_NODE_STATEMENT, keyword_token->lexeme);
...
RiftASTNode* rift1_parse_assignment_or_call(Rift1ParseEngine* engine) {
    ...
    RiftToken* identifier_token = engine->input_tokens[engine->current_position++];
    
    // Create expression node for identifier
    RiftASTNode* expression = rift_ast_node_create(AST_NODE_IDENTIFIER, identifier_token->lexeme);
...
RiftASTNode* rift1_parse_expression_statement(Rift1ParseEngine* engine) {
    ...
    RiftASTNode* expression = rift_ast_node_create(AST_NODE_EXPRESSION, token->lexeme);
