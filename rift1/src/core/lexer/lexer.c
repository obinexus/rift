...
static const RiftLexerConfig DEFAULT_CONFIG = {
...
RiftLexeme* rift_lexer_next_lexeme(RiftLexer* lexer) {
    if (!lexer || lexer->at_eof) return NULL;
    ...
    if (lexeme_length == 0) return NULL;
...
uint32_t rift_lexer_read_unicode(RiftLexer* lexer) {
    if (!lexer || !lexer->config.unicode_support) return 0;
    
    // Basic UTF-8 decoding (simplified)
    if (lexer->current_pos >= lexer->input_length) return 0;
...
RiftLexeme* rift_lexeme_create(const char* text, size_t length, RiftPosition position) {
...
void rift_lexeme_destroy(RiftLexeme* lexeme) {
    if (!lexeme) return;
    if (lexeme->text) free(lexeme->text);
