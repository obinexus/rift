...
RiftValueResolver* rift_value_resolver_create(const RiftValueResolverConfig* config) {
...
RiftTokenValue* rift_value_resolve(RiftValueResolver* resolver, 
                                  ...
                                  RiftTokenType type) {
    if (!resolver || !lexeme || !lexeme->text) return NULL;
    ...
    switch (type) {
        case RIFT_TOKEN_INTEGER:
            resolution_success = rift_value_parse_integer(
                lexeme->text, lexeme->length, 
                &token_value->value.integer_value
            );
            break;
        ...
        case RIFT_TOKEN_STRING:
            resolution_success = rift_value_parse_string(
                lexeme->text, lexeme->length,
                &token_value->value.string_value.data,
        ...
        case RIFT_TOKEN_CHAR:
            resolution_success = rift_value_parse_char(
                lexeme->text, lexeme->length, 
        ...
        case RIFT_TOKEN_BOOLEAN:
            resolution_success = rift_value_parse_boolean(
                lexeme->text, lexeme->length, 
                &token_value->value.boolean_value
            );
            break;
    ...
    if (!resolution_success) {
        ...
        snprintf(error_buffer, sizeof(error_buffer), 
                ...
                rift_token_type_name(type), 
                (int)lexeme->length, lexeme->text);
...
bool rift_value_parse_integer(const char* text, size_t length, int64_t* result) {
    if (!text || !result || length == 0) return false;
...
bool rift_value_parse_string(const char* text, size_t length, char** result, size_t* result_length) {
    if (!text || !result || !result_length || length < 2) return false;
    ...
    if (!string_content) return false;
...
bool rift_value_parse_char(const char* text, size_t length, char* result) {
    if (!text || !result || length != 3) return false;
...
bool rift_value_validate(const RiftTokenValue* token_value) {
    ...
    if (!token_value->metadata.type_validated) return false;
    
    // Check value validation
    if (!token_value->metadata.value_validated) return false;
    ...
    switch (token_value->metadata.type) {
        ...
        case RIFT_TOKEN_FLOAT:
            ...
            if (!isfinite(token_value->value.float_value)) return false;
            break;
        ...
        case RIFT_TOKEN_KEYWORD:
            ...
            if (token_value->value.string_value.length > 4096) return false;
            break;
...
const char* rift_value_get_validation_error(const RiftTokenValue* token_value) {
    return token_value ? token_value->metadata.validation_error : "Invalid token value";
}
