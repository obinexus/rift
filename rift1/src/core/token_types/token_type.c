...
static const RiftTokenPattern DEFAULT_PATTERNS[] = {
    ...
    {RIFT_TOKEN_CLASSICAL_MODE, "classical", "Classical mode directive", false, 95},
    ...
    {RIFT_TOKEN_OPERATOR_GREATER_EQUAL, ">=", "Greater or equal operator", false, 85},
...
RiftTokenTypeMatcher* rift_token_type_matcher_create(void) {
    ...
    matcher->patterns = calloc(matcher->pattern_capacity, sizeof(RiftTokenPattern));
        ...
        if (pattern->is_regex) {
            ...
            if (pattern_len == lexeme->length) {
                ...
                if (matcher->case_sensitive) {
                    ...
                    cmp_result = strncasecmp(pattern->pattern, lexeme->text, lexeme->length);
                }
...
bool rift_pattern_match(const char* pattern, const char* text, size_t text_length) {
    ...
    if (strcmp(pattern, "^[0-9]+\\.[0-9]+$") == 0) {
        ...
        for (size_t i = 0; i < text_length; i++) {
            if (text[i] == '.') {
                if (found_dot) return false; // Multiple dots
                found_dot = true;
            } else if (isdigit(text[i])) {
                if (!found_dot) has_digits_before = true;
                else has_digits_after = true;
            } else {
...
const char* rift_token_type_name(RiftTokenType type) {
    switch (type) {
        ...
        case RIFT_TOKEN_BOOLEAN: return "BOOLEAN";
        case RIFT_TOKEN_IDENTIFIER: return "IDENTIFIER";
        case RIFT_TOKEN_KEYWORD: return "KEYWORD";
        case RIFT_TOKEN_OPERATOR_ASSIGN: return "ASSIGN";
        case RIFT_TOKEN_OPERATOR_PLUS: return "PLUS";
        case RIFT_TOKEN_OPERATOR_MINUS: return "MINUS";
        case RIFT_TOKEN_OPERATOR_MULTIPLY: return "MULTIPLY";
        case RIFT_TOKEN_OPERATOR_DIVIDE: return "DIVIDE";
        case RIFT_TOKEN_DELIMITER_SEMICOLON: return "SEMICOLON";
        case RIFT_TOKEN_BRACKET_OPEN_PAREN: return "OPEN_PAREN";
        case RIFT_TOKEN_BRACKET_CLOSE_PAREN: return "CLOSE_PAREN";
        case RIFT_TOKEN_CLASSICAL_MODE: return "CLASSICAL_MODE";
        case RIFT_TOKEN_QUANTUM_MODE: return "QUANTUM_MODE";
        case RIFT_TOKEN_RIFT_DIRECTIVE: return "RIFT_DIRECTIVE";
