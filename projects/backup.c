...
// Helper function to generate unique state IDs
...
// Create a new state with a regex pattern
State *
...
// Check if a state’s regex matches a given text
bool
...
void
automaton_destroy(RegexAutomaton *automaton)
{
    if (!automaton)
        return;
...
State *
automaton_add_state(RegexAutomaton *automaton, const char *pattern, bool is_final)
{
...
bool
automaton_add_transition(RegexAutomaton *automaton, State *from, const char *pattern, State *to)
{
    if (!automaton || !from || !pattern || !to)
        return false;
    // Expand capacity if needed
    if (automaton->transition_count >= automaton->transition_capacity) {
        ...
        Transition **new_transitions =
            realloc(automaton->transitions, sizeof(Transition *) * new_capacity);
        if (!new_transitions)
            return false;
...
State *
automaton_get_next_state(RegexAutomaton *automaton, const char *input)
{
    if (!automaton || !automaton->current_state || !input)
        return NULL;
    for (size_t i = 0; i < automaton->state_count; i++) {
...
// IR Generator: Process tokens using the automaton
IRGenerator *
...
void
ir_generator_destroy(IRGenerator *generator)
{
    if (!generator)
        return;
...
TokenNode *
ir_generator_process_token(IRGenerator *generator, const char *token)
{
    if (!generator || !token)
        return NULL;
...
// Example usage: Create a simple lexer using LibRift
void
create_simple_lexer(void)
{
    RegexAutomaton *automaton = automaton_create();

    // Add states with regex patterns
    ...
    // Create IR generator using the automaton
